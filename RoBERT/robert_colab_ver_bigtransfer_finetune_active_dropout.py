...
load_model_path = "bigtransfer_roberta_sentiment_model_5.pt" # EDIT
load_optimizer_path = "bigtransfer_roberta_sentiment_optimizer_5.pt" # EDIT
output_model_name = 'bigtransfer5_finetune_active_dropout_roberta_sentiment_model' # EDIT
output_optimizer_name = 'bigtransfer5_finetune_active_dropout_roberta_sentiment_optimizer' # EDIT
...
# 원래 데이터셋
train = pd.read_csv(data_dir.joinpath('train_plus.csv')) # EDIT
...
if load_model:
    model.load_state_dict(torch.load(load_model_path, map_location=torch.device(device)), strict=False)
model.to(device)
...
# if load_model:
#     optimizer.load_state_dict(torch.load(load_optimizer_path, map_location=torch.device(device)))
...
# valid(model)
predictions = MC_soft_predict(model, testing_loader, 100)
submission = pd.DataFrame({'Id' : range(len(predictions)), 'Category' : predictions})
submission.to_csv('bigtransfer5_finetune_active_dropout_submission_30.csv', index=False) # EDIT
